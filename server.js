...
//Anima
var anima = new GmAnima(1000, function() {

	//將frame填入buff
	var frame = anima.getNowFrame();
	for (var i = 0; i < 6; i++) {
		var data = GmProtocal.generateData(i + 1, 2, frame[i], frame[i]);
		for (var j = 0; j < data.length; j++) buff.push(data[i]);
		console.log(data);
	}
});
...
router.post('/form', function(req, res, next) {
	...
	switch (mode) {
		...
		case 1:
			...
			greenMan.setShiftPosition(-13, 0);
			//startScroll(1, 0, greenMan.rowPixels[0].length * 2 + 2, 1300, protoaclScrolling);
			var frames = [];
			var frameCount = greenMan.rowPixels[0].length + 14;
